...
$('#logform').on('submit', function(e) {
    ...
    // FOR CLASSROOM ACCESS (Instructors): Otherwise show subject selection
        ...
        // In your loadInstructorSubjects function, update the success handler:
success: function(response) {
    try {
        ...
        if (data.status === 'success') {
            ...
            $('#subjectList').html(`<tr><td colspan="5" class="text-center text-danger">${data.message || 'Error loading subjects'}</td></tr>`);
        }
    } catch (e) {
        console.error('Error parsing subjects:', e, response);
        $('#subjectList').html('<tr><td colspan="5" class="text-center text-danger">Error loading subjects. Please try again.</td></tr>');
    }
},
                error: function(xhr) {
